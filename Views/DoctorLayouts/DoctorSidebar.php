<aside>
	<?php
		if(isset($resultReceptionPatient)) {
...
<script>
	...
	const recevingPatientContainer = document.querySelector('#receiving--patient__container');
	console.log(recevingPatientContainer);
	

	function renderWaitingExamList(data) {
		console.log(data);
		let htmls = [];
		let recevingPatient = <?= $resultReceptionPatient ?>;
		console.log(recevingPatient);
			data.forEach(element => {
				if(element.status == 1) {
					recevingPatientContainer.querySelector('#createAt').innerText = element.createAt;
					recevingPatientContainer.querySelector('#patientName').innerHTML = `<b>Trẻ:</b> ${element.patientName}`;
					recevingPatientContainer.querySelector('#patientCode').innerHTML = `<b>ID: ${element.patientCode}</b>`;
